...
/// Shown before Config All / Config Selected to let the user choose
/// between immediate execution and staggered batching (to avoid
/// tripping breakers from simultaneous reboots after config changes).
class ConfigBatchDialog extends StatefulWidget {
  final int totalMiners;
  final String title;

  const ConfigBatchDialog({
    super.key,
  ...
  @override
  State<ConfigBatchDialog> createState() => _ConfigBatchDialogState();
}

class _ConfigBatchDialogState extends State<ConfigBatchDialog> {
  ...
  Future<void> _saveSettingsAndPop(String result) async {
    final batchSize =
        (int.tryParse(_batchSizeController.text) ?? _savedBatchSize).clamp(1, 50);
    final batchDelay =
        (int.tryParse(_batchDelayController.text) ?? _savedBatchDelay).clamp(1, 60);
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Row(
        children: [
          Icon(Icons.build, size: 20, color: Theme.of(context).colorScheme.primary),
          SizedBox(width: 8),
      ...
      content: SizedBox(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Configure staggered execution to avoid power surges from simultaneous reboots, or apply to all miners at once.',
